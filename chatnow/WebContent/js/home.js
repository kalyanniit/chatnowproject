var chat=angular.module("chat",['ngRoute','ngCookies']);
...
chat.run( function ($rootScope, $location,$cookieStore, $http) {
...
chat.controller("chatController",function($scope,$http,ChatService,$rootScope)
		{
	...
	$scope.messages = [];
	  ...
	  ChatService.receive().then(null, null, function(message) {
		...
		chat.controller("JobsController",function($scope,$http)
				{
			...
			$scope.addJ=function()
			{
				...
				var res=$http.post("http://localhost:8089/chat/addJobs",jobs);
				...
				chat.controller("loginController",['$cookieStore','$scope','$http','$location','$rootScope',function($cookieStore,$scope,$http,$location,$rootScope)
						{
					...
					$http.post("http://localhost:8089/chat/authenticate",logi).then(function(response)
							{
				...
				chat.controller("blogController",function($scope,$http,$rootScope)	
					...
					$http.get("http://localhost:8089/chat/viewMyBlogs/"+$rootScope.uname)
							 ...
							 $scope.newBlog={};
								...
								$scope.addBlog=function(newBlog)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:8089/chat/addBlog',dataObj);
									 $http.get("http://localhost:8089/chat/viewMyBlogs/"+$rootScope.uname)
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:8089/chat/updateBlog",edit);
					$http.get("http://localhost:8089/chat/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				$http.post("http://localhost:8089/chat/deleteBlog",del);
					$http.get("http://localhost:8089/chat/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				chat.controller("adminBlogController",function($scope,$http,$rootScope)	
					...
					console.log(" in adminblog controller");
							 ...
							 $http.get("http://localhost:8089/chat/viewBlogs")
							    .then(function (response) {
				...
				chat.controller("adminJobsController",function($scope,$http,$rootScope)	
					...
					console.log(" in jobs controller");
							 ...
							 $http.get("http://localhost:8089/chat/viewJobs")
							    .then(function (response) {
							 ...
							 $scope.newJobs={};
								...
								$scope.addJobs=function(newJobs)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:8089/chat/addJobs',dataObj);
									 $http.get("http://localhost:8089/chat/viewJobs")
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:8089/chat/updateJob",edit);
					 $http.get("http://localhost:8089/chat/viewJobs")
					    .then(function (response) {
				...
				$http.post("http://localhost:8089/chat/deleteJob",del);
					 $http.get("http://localhost:8089/chat/viewJobs")
					    .then(function (response) {
				...
				chat.controller('logoutController',function($scope,$rootScope,$http,$cookieStore)		
						{
					$http.post("http://localhost:8089/chat/logout/"+$rootScope.uname);
				...
				chat.controller("userJobsController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
				...
				chat.controller("adminForumController",function($scope,$http,$rootScope)	
					...
					console.log(" in adminforum controller");
							 ...
							 $http.get("http://localhost:8089/chat/viewForums")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.addForum=function(newForum)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:8089/chat/addForum',dataObj);
									 $http.get("http://localhost:8089/chat/viewForums")
				...
				$scope.approveForum=function()
				{
					...
					$http.put("http://localhost:8089/chat/updateForum",edit);
					 $http.get("http://localhost:8089/chat/viewForums")
					    .then(function (response) {
				...
				chat.controller("userForumController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log(" in forum controller");
							 ...
							 $http.get("http://localhost:8089/chat/viewForum")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.answer=function(forum)
								{
									...
									answer:$scope.answerforum.answer
									 ...
									 $http.post('http://localhost:8089/chat/answerForum',ans);
									$http.get("http://localhost:8089/chat/viewForum")
								    .then(function (response) {
								...
								$scope.viewanswers=function(forum)
								{
									...
									console.log("forumid:"+$scope.answerforum.forum_id);
									 $http.get("http://localhost:8089/chat/viewAnswers/"+$scope.answerforum.forum_id)
									    .then(function (response) {
				...
				$http.post("http://localhost:8089/chat/deleteForum",del)
				...
				chat.controller("userHomeController",function($scope,$http,$rootScope)	
						{	
					...
					console.log("name in  findfriends:"+$rootScope.uname);
							 $http.get("http://localhost:8089/chat/findFriends/"+$rootScope.uname)
							    .then(function (response) {
					...
					$scope.addfriend=function(user)
					{
						...
						$http.post("http://localhost:8089/chat/addFriend/",fr);
						
					}
					
						});
				chat.controller("friendslistController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("name in  friendslist:"+$rootScope.uname);
							 $http.get("http://localhost:8089/chat/viewFriends/"+$rootScope.uname)
							    .then(function (response) {
				...
				chat.controller("allblogsController",function($scope,$http,$rootScope,$route)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("username in allblog controller:"+$rootScope.uname);
							 $http.get("http://localhost:8089/chat/viewAllBlogs")
							    .then(function (response) {
							 ...
							 { 
						       ...
						       var like=
								...
								console.log("postedby:"+$rootScope.uname);
								 $http.put('http://localhost:8089/chat/updateBlog',like);
								 $http.get("http://localhost:8089/chat/viewBlogs")
								    .then(function (response) {
					 ...
					 $http.get("http://localhost:8089/chat/viewComments/"+$scope.viewcomments.blog_id)
								    .then(function (response) {
						...
						$scope.comment=function(allblogs)
						{
							...
							$http.post('http://localhost:8089/chat/addComment',comment);
							 $http.get("http://localhost:8089/chat/viewBlogs")
							    .then(function (response) {
